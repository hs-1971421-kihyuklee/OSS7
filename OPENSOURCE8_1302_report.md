...
### 4. 사용된 오픈소스 소프트웨어 소개
   ...
   오픈카트가 사용되기 위해 웹 서버(Apache 권장), PHP 5.4+, 데이터베이스(MySQLi 권장)가 필요합니다. 이후에 다운로드를 받아 사용하면 되는데 홈페이지에 가면 설명영상과 함께 자세히 설명되어있습니다.  
   다양한 언어와 통화, 제품 리뷰, 많은 종류의 결제 게이트웨이가 사용 가능합니다.
    
   웹 서버의 오픈카트를 활용하여 제품의 주문,결제 등 제품을 구매하는 전반적인 행위를 하게 됩니다.  
   판매 보고서와 리뷰, 고객정보를 DB에 받아서 정보를 처리하는데 사용할 수 있습니다.
