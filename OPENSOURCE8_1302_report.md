...
목적과 용도에 부합하는 컴퓨터 제품 들을 확인 혹은 비교하고, 해당 관련 상품들을 추천해주는 서비스입니다.
국내 뿐 아닌 해외에서도 구매가 가능하고 실시간으로 배송 정보(위치)를 확인 가능합니다.

-----

### 2. 유사 서비스 분석

다나와는 컴퓨터 주요부품 가격비교로 시작해 현재 전 카테고리를 망라하는 종합 가격비교사이트로 최저가뿐 아니라 전문적이고 다양한 쇼핑정보를 제공합니다.
다나와 가격비교 서비스는 스포츠, 가구, 식품, 자동차, 여행까지 카테고리를 계속 확장하고 있습니다.
정교한 검색 옵션과 믿을 수 있는 최저가 정보를 바탕으로 구매가이드, 뉴스, 동영상 리뷰 등 다양한 자체 콘텐츠를 제공하여 포털이 넘볼 수 없는 강력한 전문성이 특징입니다.
오픈소스로 dsearch 시리즈, service- management 등 범용적인 소프트웨어를 공개합니다.
...
### 4. 사용된 오픈소스 소프트웨어 소개
   ...
   OpenStreetMap은 OpenStreetMap으로 누구나 참여할 수 있는 오픈 소스 방식의 무료 지도 서비스입니다.
   osmdroid는 OpenStreetMap-Tools for Android로 Apache-2.0 license로 안드로이드의 맵뷰(v1 API) 클래스를 대체하는 제품입니다. 
   수많은 온라인 및 오프라인 타일 소스를 지원하는 모듈식 타일 공급자 시스템과 플로팅 아이콘, 추적 위치 및 도면 모양 지원하는 오버레이를 포함합니다.
   

   배송이 될 때 위치가 명확하지 표시되지 않는 순간들이 있습니다.
   저희가 구현할 서비스에서는 Osmdroid로 위치를 보다 명확히 파악하여 추적하기 용이하게 하는 것이 목표입니다.
   ...
   Gorse는 라이선스는 Apache License 2.0이고, Go로 제작하는 오픈소스 추천 시스템입니다.  
   ...
   Elasticsearch의 사용분야 로는 애플리케이션 검색, 웹사이트 검색, 엔터프라이즈 검색, 로깅과 로그 분석, 인프라 메트릭과 컨테이너 모니터링, 애플리케이션 성능 모니터링, 위치 기반 정보 데이터 분석 및 시각화,보안 분석, 비즈니스 분석 등이 있습니다.

   Elasticsearch의 작동법을 소개하겠습니다.  
   ...
   Elasticsearch 프로그래밍 지원 언어로는 자바, 자바스크립트(Node.js),Go,.NET(C#),PHP,Perl,Python,Ruby 등이 있습니다.
   ...
   Elasticsearch 기능 Enterprise Search에 대해 설명하겠습니다.  
   1. 인사이트를 행동으로 전환  
   ...
   Elasticsearch를 사용하는 기업으로는 H-E-B, General Motors, Office Depot 등이 있습니다.
   ...
   Apache HTTP Server의 독특한 기능으로는
      IPv6, XML, FTP, 펄, 루아, PHP, 대역폭 스로틀 링, WebDAV, 로드 균형 조정, HTTP / 2, .htaccess, 다중 요청 처리 모드(MPM),URL 재작성,세션 추적,IP 주소를 기반으로 한 위치       정보등이 있습니다
   ...
   Redis는 Remote dictionary server의 약자 입니다.  
